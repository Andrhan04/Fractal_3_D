...
def draw_trap(color: str, exp_id: int):
    global axes
    trap_file = f"Sowing\\Trap\\Traps_{exp_id}.txt"
    if os.path.isfile(trap_file):
        id, x, y, z, radius = [], [], [], [], []
        with open(file=trap_file, mode='r') as file:
            for line in file:
                parts = line.split()
                if len(parts) < 5:
                    continue
                ...
                radius.append(float(parts[4]))
        
        # Отрисовка сфер для каждой ловушки
        for i in range(len(x)):
            # Создание сферы
            axes.scatter(x[i], y[i], z[i], color=color, s=5)
