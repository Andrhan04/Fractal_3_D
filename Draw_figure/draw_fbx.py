...
def draw_point(color, exp_id, axes, points_store, project_root, point_size=20):
    ...
    if os.path.isdir(experiments_dir):
        ...
        if os.path.isfile(filepath):
            ...
            axes.scatter(x, y, z, color=color, s=point_size)
...
def export_obj(path, cylinders, points):
    ...
    color_map = {
        ...
        'g': (0.0, 1.0, 0.0),      # зеленый для точек
    }
    ...
    for i, (bottom, top, color) in enumerate(cylinders):
        n = bottom.shape[0]
        base = len(vertices) + 1
        ...
        # Добавляем материал для этого цилиндра
        material_name = f"material_{color}"
        materials.append(f"usemtl {material_name}")
    ...
    if points:
        ...
        for px, py, pz in points:
            ...
            vertices.append(f"v {px:.6f} {py:.6f} {pz + point_radius:.6f}")  # top
            vertices.append(f"v {px + point_radius:.6f} {py:.6f} {pz:.6f}")  # right
            vertices.append(f"v {px:.6f} {py + point_radius:.6f} {pz:.6f}")  # front
            vertices.append(f"v {px - point_radius:.6f} {py:.6f} {pz:.6f}")  # left
            vertices.append(f"v {px:.6f} {py - point_radius:.6f} {pz:.6f}")  # back
            vertices.append(f"v {px:.6f} {py:.6f} {pz - point_radius:.6f}")  # bottom
            
            material_faces.append(("material_g", [
                f"f {base + 0} {base + 1} {base + 2}",
                f"f {base + 0} {base + 2} {base + 3}",
                f"f {base + 0} {base + 3} {base + 4}",
                f"f {base + 0} {base + 4} {base + 1}",
                f"f {base + 5} {base + 2} {base + 1}",
                f"f {base + 5} {base + 3} {base + 2}",
                f"f {base + 5} {base + 4} {base + 3}",
                f"f {base + 5} {base + 1} {base + 4}",
            ]))
